...
- The current source lock still has ESLint 10 peers rejected by three plugins bundled in `eslint-config-next` whose ranges stop at ESLint 9. Source qualification refuses that graph; it does not silently grandfather the peers. Production-to-main also includes adoption tooling requiring a separately reviewed bootstrap. Local synthetic proof tests do not resolve either gap or prove hosted CI, scanner availability, installation or a maintenance pilot.
